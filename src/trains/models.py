...
class Train(models.Model):
    ...
    from_sity = models.ForeignKey(City, on_delete=models.CASCADE,
                                  # null=True, blank=True,
    ...
    def __str__(self):
        return f'Поезд номер:{self.name} из города {self.from_sity}'
    ...
    def clean(self):
        if self.from_sity == self.to_city:
            raise ValidationError('Изменить город прибытия')
        qs = Train.objects.filter(from_sity=self.from_sity,
